...
function costPf(report: IctExperimentReport, costPips: number) {
  return report.execution_cost_simulation.find((row) => row.cost_pips === costPips)?.profit_factor ?? 0;
}

function costNet(report: IctExperimentReport, costPips: number) {
  return report.execution_cost_simulation.find((row) => row.cost_pips === costPips)?.net_profit ?? 0;
}
...
function main() {
  ...
  for (const [symbol, path] of Object.entries(FOREX_FILES)) {
    try {
      ...
      reports.push(...runIctExperimentSuite(symbol, rows, INITIAL_CAPITAL));
    } catch (error) {
  ...
  const output: SuiteOutput = {
    suite_name: "Forex ICT/FVG v4 Experiment Suite",
    suite_version: "v4",
  ...
  writeFileSync(`${OUT_DIR}/ict_fvg_experiment_summary.json`, JSON.stringify(output, null, 2), "utf8");
  writeCsv(`${OUT_DIR}/ict_fvg_experiment_summary.csv`, [
    summaryHeaders,
    ...reports.map(summaryRow),
  ]);
  writeCsv(`${OUT_DIR}/ict_fvg_best_variants.csv`, [
    summaryHeaders,
  ...
  if (bestVariant) {
    ...
    writeCsv(`${OUT_DIR}/ict_fvg_best_variant_trades.csv`, [
      tradeHeaders,
      ...tradeRows(bestVariant.trades),
    ]);
    writeFileSync(`${OUT_DIR}/ict_fvg_best_variant_report.json`, JSON.stringify(bestVariant, null, 2), "utf8");
  }
    ...
    JSON.stringify(
      {
        ...
        best_variant: bestVariant
          ...
          : null,
        files: [
          "ict_fvg_experiment_summary.csv",
          "ict_fvg_experiment_summary.json",
          "ict_fvg_best_variants.csv",
          "ict_fvg_best_variant_trades.csv",
          "ict_fvg_best_variant_report.json",
