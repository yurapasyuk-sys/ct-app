...
type Regime = "none" | "trend" | "counter";
type Timeframe = "1h" | "4h";
type ExitMode = "mean" | "opposite_band";
...
function regimeAllowed(regime: Regime, direction: Direction, close: number, ema: number | null) {
  if (regime === "none") return true;
  if (ema == null) return false;
  if (regime === "trend") return direction === "long" ? close > ema : close < ema;
...
function run(symbol: string, rows: Kline[], config: Config) {
  ...
  const bands = buildBands(rows, config.bbPeriod, config.deviation);
  const warmup = Math.max(config.bbPeriod, config.atrPeriod, config.emaPeriod) + 2;
  const trades: Trade[] = [];
  ...
  for (let index = warmup; index < rows.length; index += 1) {
    ...
    if (!regimeAllowed(config.regime, direction, signal.close, ema[signalIndex])) continue;
    ...
    const takeProfit = config.exitMode === "mean" ? band.mean : direction === "long" ? band.upper : band.lower;
    if ((direction === "long" && takeProfit <= entryPrice) || (direction === "short" && takeProfit >= entryPrice)) continue;
...
function buildConfigs() {
  ...
  for (const timeframe of ["1h", "4h"] as const) {
    for (const bbPeriod of [20, 40, 80]) {
      for (const deviation of [1.25, 1.5, 2]) {
        for (const stopAtr of [0.75, 1, 1.5]) {
          for (const maxHoldBars of timeframe === "1h" ? [24, 48, 96] : [12, 24, 48]) {
            for (const directionMode of ["all", "long_only", "short_only"] as const) {
              for (const regime of ["none", "trend", "counter"] as const) {
                for (const exitMode of ["mean", "opposite_band"] as const) {
                  configs.push({
                    timeframe,
                    bbPeriod,
                    deviation,
...
async function main() {
  ...
  writeCsv(`${OUT_DIR}/fx_yahoo_bb_atr_rows.csv`, rows);
  writeCsv(`${OUT_DIR}/fx_yahoo_bb_atr_summary.csv`, [
    Object.keys(summary[0] ?? { variant: "" }),
    ...summary.map((row) => Object.values(row)),
  ]);
  writeFileSync(`${OUT_DIR}/fx_yahoo_bb_atr_summary.json`, JSON.stringify(summary, null, 2), "utf8");
